export const tablePropsTemplate = `import * as R from 'ramda'
import { PaginationProps } from 'infrad/lib/pagination'
import { PAGE, PAGE_SIZE } from 'generated-tables/util/constants'
...
export const getTableProps = (params: {
  ...
  const { pagination, filters, sorter, searchKeys, searchValue } = params
  ...
  const searchBy = searchKeys?.reduce((acc, curr) => {
    const currSearchBy =
      getFilterUrlParam({
        searchValue: getFilterItem(curr, searchValue, FilterTypes.CONTAIN),
      }) ?? ''
    return acc + currSearchBy
  }, '')
