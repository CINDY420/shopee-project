...
/** Filter based on request parameters, temporarily only support request */
function paramsFilter(item, filter) {
    var itemRequest = item === null || item === void 0 ? void 0 : item.request;
    var filterRequest = filter === null || filter === void 0 ? void 0 : filter.request; // This line is to solve the ts2532 error
...
function useResponseData(modelName, filter) {
    var reduxData = (0, react_redux_1.useSelector)(function (state) {
        return (state.$$rapperResponseData && state.$$rapperResponseData[modelName]) || [];
    });
    var initData = getFilteredData(reduxData, filter);
    var _a = (0, react_1.useState)(initData.id), id = _a[0], setId = _a[1];
    var _b = (0, react_1.useState)(initData.response || undefined), filteredData = _b[0], setFilteredData = _b[1];
    var _c = (0, react_1.useState)(initData.isPending || false), isPending = _c[0], setIsPending = _c[1];
    var _d = (0, react_1.useState)(initData.errorMessage), errorMessage = _d[0], setErrorMessage = _d[1];
    (0, react_1.useEffect)(function () {
        /** Filter out the latest eligible data */
...
/** Determine whether to send a request for anti-shake */
var judgeDebounce = function (debounce, requestTime) {
...
function useAPICommon(_a) {
    ...
    var _e = (0, react_1.useState)(requestParams), requestParamsMemo = _e[0], setRequestParamsMemo = _e[1];
    var _f = (0, react_1.useState)(undefined), filteredData = _f[0], setFilteredData = _f[1];
    var _g = (0, react_1.useState)(false), isPending = _g[0], setIsPending = _g[1];
    var _h = (0, react_1.useState)(undefined), errorMessage = _h[0], setErrorMessage = _h[1];
    /** In most cases, requestParams are objects. In order to avoid unnecessary re-renders, they are cached */
    (0, react_1.useEffect)(function () {
    ...
    (0, react_1.useEffect)(function () {
        /** Manual mode does not go global, only returns the data of its own fetch */
        if (mode === 'manual') {
            return;
        }
        /** Filter out the latest eligible data */
    ...
    var request = (0, react_1.useCallback)(function (req, extra) { return __awaiter(_this, void 0, void 0, function () {
...
function assignData(_a) {
    ...
    if (isPending === true) {
        /** Only store the most recent maxCacheLength data */
        if (maxCacheLength !== Infinity && data.length >= maxCacheLength) {
            data = newState[interfaceKey].slice(data.length - maxCacheLength + 1);
        }
        newState[interfaceKey] = __spreadArray(__spreadArray([], data, true), [{ id: id, requestTime: requestTime, request: request, isPending: isPending }], false);
    }
    else {
...
function rapperEnhancer(config) {
    ...
    var _a = (config || {}).debounce, debounce = _a === void 0 ? 1000 : _a;
    ...
    return function (next) { return function (reducers) {
        ...
        var store = next.apply(void 0, __spreadArray([reducers], args, false));
        /** redefine reducers */
        var newReducers = function (state, action) {
            if (state && !state.$$rapperResponseData) {
                throw Error('Rapper initialization configuration failed, rootReducer should be added to rapperReducers, please check the demo configuration for details: https://www.yuque.com/rap/rapper/react-install#rYm5X');
            }
            if (!action.hasOwnProperty('type')) {
                return reducers(state, action);
            }
            switch (action.type) {
                /** request success，update store */
                ...
                switch (_c.label) {
                    case 0:
                        ...
                        if (cacheDataPending.length >= maxCacheLength) {
                            errorMessage = "\u5F53\u524D\u914D\u7F6E\u7684\u7F13\u5B58\u533A\u6700\u591A\u652F\u6301".concat(maxCacheLength, "\u4E2A\u5E76\u53D1\u8BF7\u6C42\uFF0C\u5982\u9700\u8981\u66F4\u5927\u7684\u7F13\u5B58\u533A\uFF0C\u8BF7\u4FEE\u6539 maxCacheLength \u53C2\u6570");
                            store.dispatch({
                                type: FAILURE,
                        ...
                        /**
                         * Anti-shake treatment, two conditions：
                         * 1、same request parameters
                         * 2、The request interval is less than debounce
                         */
                        if (cacheData
                            .filter(function (item) { return (0, lodash_1.isEqual)(item.request, params); })
                            .some(function (item) { return !judgeDebounce(debounce, item.requestTime); })) {
                            return [2 /*return*/];
                        }
