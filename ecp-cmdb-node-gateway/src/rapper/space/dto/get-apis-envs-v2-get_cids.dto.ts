/* md5: 9e539937d29dc81f28197c3368f390fc */
/* Rap repository id: 225 */
/* Rapper version: 2.1.13 */
/* eslint-disable */
/* tslint:disable */
// @ts-nocheck

/**
 * This file is automatically generated by Rapper to synchronize the Rap platform interface, please do not modify
 * Rap repository url: http://rap.shopee.io/repository/editor?id=225
 */

/**
 * Interface name：get cids
 * Rap url: http://rap.shopee.io/repository/editor?id=225&mod=1189&itf=11439
 */

import {ValidateNested, IsArray, IsNotEmpty, IsString, IsNumber, IsBoolean} from 'class-validator'
import {Type} from 'class-transformer'

export class Item449110 {
  @IsNotEmpty()
  @IsString()
  'alias_name': string

  @IsNotEmpty()
  @IsNumber()
  'enum_value': number

  @IsNotEmpty()
  @IsString()
  'name': string

  @IsNotEmpty()
  @IsBoolean()
  'visible': boolean
}

export class GetApisEnvsV2GetCidsReqDto {}
export class GetApisEnvsV2GetCidsResDto {
  @IsNotEmpty()
  @ValidateNested()
  @Type(() => Item449110)
  @IsArray()
  'result': Item449110[]
}
