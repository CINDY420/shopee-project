...
import { ContentWrapper } from 'src/components/DeployConfig/ListView/Strategy/style'
import {
  DeployConfigContext,
  FormType,
  StrategyType,
...
const StrategyA: React.FC<IStrategyAProps> = ({ countryAzsOptions }) => {
  ...
  const notice = (
    <>Only the following Components will be affected by Strategy: {nonBromoComponent}</>
  )
  ...
  const Columns = [
        ...
        return (
          <Form.Item
            shouldUpdate={(prevValue, curValue) =>
              prevValue.override?.[name]?.cid !== curValue.override?.[name]?.cid
            }
          >
            {({ getFieldValue }) => {
              ...
              return (
                <Form.Item
                  {...restField}
                  name={[name, 'idcs']}
                  rules={[COMPULSORY_FIELD_VALIDATOR]}
                >
    ...
    },
    {
      ...
      width: '17%',
      render: (_: unknown, record: FormListFieldData) => (
        <Form.Item name={[record.name, 'data', 'name']} initialValue={StrategyType.RECREATE}>
          <AutoDisabledSelect>
            <Select.Option value={StrategyType.ROLLING_UPDATE}>RollingUpdate</Select.Option>
            <Select.Option value={StrategyType.RECREATE}>Recreate</Select.Option>
          </AutoDisabledSelect>
        </Form.Item>
      ),
    },
      ...
      render: (_: unknown, record: FormListFieldData) => (
        <Form.Item
          ...
          shouldUpdate={(prevValues, curValues) =>
            prevValues.override?.[record.name]?.data?.name !==
            curValues.override?.[record.name]?.data?.name
          }
        >
          {({ getFieldValue }) => {
            const type = getFieldValue(['override', record.name, 'data', 'name'])
            return type === StrategyType.ROLLING_UPDATE ? (
              <Form.Item
      ...
      >
        <ContentWrapper>
          <Form.Item
            ...
            name="k8sStrategy"
            labelCol={{ span: 4 }}
            initialValue={StrategyType.RECREATE}
          >
            <AutoDisabledRadioGroup>
              <Radio value={StrategyType.ROLLING_UPDATE}>RollingUpdate</Radio>
              <Radio value={StrategyType.RECREATE}>Recreate</Radio>
            </AutoDisabledRadioGroup>
          </Form.Item>
          <Form.Item
            noStyle
          ...
          >
            {({ getFieldValue }) => {
              ...
              return strategyType === StrategyType.ROLLING_UPDATE ? (
                <>
                  <Form.Item
                    name="max_surge"
                    label="Max Surge"
                  ...
                  >
                    <AutoDisabledInput
                      placeholder="Input number or percentage"
                      style={{ width: 212 }}
                    />
                  </Form.Item>{' '}
                </>
