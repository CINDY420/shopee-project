...
import { FILTER_TYPES, getFilterItem, getFilterUrlParam } from 'helpers/queryParams'
...
export interface IAntdTableProps {
  fetchFn: any
  filterTypeLookup?: Record<string, FILTER_TYPES>
  orderDefault?: string
 ...
 */
const useAntdTable = <TRecordType>({
  fetchFn,
  filterTypeLookup = {},
  orderDefault = '',
  ...
  const handleTableChange = React.useCallback<IAntdTableOnChange<TRecordType>>(
    (pagination, filters, sorter) => {
      ...
      Object.entries(filters ?? {}).forEach(([key, value]) => {
        const filterType = filterTypeLookup[key] || FILTER_TYPES.EQUAL
