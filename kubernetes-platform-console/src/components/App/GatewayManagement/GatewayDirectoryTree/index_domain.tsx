...
import { LOAD_BALANCE, INGRESS } from 'constants/routes/gateway/name'
...
const paths = ['gateways', 'groups', 'projects']
...
const generateServicesTree = (rawData: any) => {
  const generateTreeNode = (data, deep = 0, prefix?: string) => {
    ...
    return resources.map(resource => {
      const { name } = resource
      const resourceName = `${currentKey}/${name}`
...
const getDefaultTreeData = (treeData, paths?: string[]) => {
  ...
  if (paths.length) {
    ...
    treeNodeData = treeData.find(item => item.title === path)
  } else {
  ...
  React.useEffect(() => {
    ...
    paths.forEach(path => {
      ...
      if (match && match.isExact) {
        const { params } = match
        const { gatewayName = 'Services', groupName } = params as any
        const paths = groupName ? [gatewayName, groupName] : [gatewayName]
