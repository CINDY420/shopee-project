...
import { useRemoteRecoil, AsyncFn, IAsyncState, AsyncStatus } from '../useRecoil'
import * as useMountedState from '../useMountedState'

let fn: jest.Mock<any> = null
let hook: RenderHookResult<IAsyncState<any>, AsyncFn> = null

// mocking dependencies
let useMountedStateMock: jest.SpyInstance = null
let useSetRecoilStateMock: jest.SpyInstance = null
...
describe('fn and state', () => {
  it('fn that is promiss shoulde be invoked', async () => {
    ...
    expect(state.status).toBe(AsyncStatus.FINISHED)
  })
  ...
  it('fn that return error shoulde produce error', async () => {
    jest.fn().mockRejectedValueOnce({ message: 'error' })
    hook = renderHook(() => useRemoteRecoil(fn, recoilState))
...
describe('clean', () => {
  it('shoulde reset status', () => {
    ...
    expect(state.status).toBe(AsyncStatus.INITIAL)
  })
})
