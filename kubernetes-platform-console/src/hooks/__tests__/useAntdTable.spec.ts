import useAntdTable, { IAntdTableProps, IAntdTableResult } from '../table/useAntdTable'
...
let fetchFn: jest.Mock<any> = null
let hook: RenderHookResult<IAntdTableProps, IAntdTableResult<any>> = null
// mocking dependencies
let getFilterUrlParamMock: jest.SpyInstance = null
let formatSorterMock: jest.SpyInstance = null
let formatPaginationMock: jest.SpyInstance = null
...
const sorter: SorterResult<any>[] = [{ column: { title: 'Time', dataIndex: 'time', key: 'name' }, order: 'descend' }]
...
describe('filterTypeLookup', () => {
  const nameUrlParams = {
    key: 'name',
    type: queryParams.filterTypes.equal,
    value: ['a']
  }
  ...
  it('default filter type should be filterTypes.equal if filterTypeLookup is not provided', () => {
    ...
    act(() => handleTableChange(pagination, filters, sorter, extra))
  ...
  it('filter type should be taken from provided filterTypeLookup if provided', () => {
    fetchFn = jest.fn()
    hook = renderHook(() => useAntdTable({ fetchFn, filterTypeLookup: { name: queryParams.filterTypes.contain } }))
    ...
    expect(getFilterUrlParamMock).toBeCalledWith({
      name: {
        ...nameUrlParams,
        type: queryParams.filterTypes.contain
      }
    })
  })
})
...
describe('refresh', () => {
  ...
  beforeEach(() => {
    ...
    act(() => handleTableChange(nextPagination, filters, sorter, extra))
  })
